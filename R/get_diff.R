                     ...
                     verbose = TRUE, ...)
{
  ...
  else res <- get_diff_pwc (dds, factor = factor, grp1 = grp1, grp2 = grp2,
                       ...
                       verbose = TRUE, ...)
{
  ...
  step_name <- tolower(paste0(grp2, "_vs_", grp1))
  if(verbose) message("- Comparison ", step_name, ": Progress = ", 1, "/", length(grp1))

  res <- .results(dds, contrast = c(factor, grp1, grp2), alpha = 0.05, ...)
  ...
  significance <- as.numeric(ifelse(is.na(res$padj), FALSE,
                                    res$padj <= alpha & fcsOk &
                                      res$detection_call == 1))*sign(res$log2FoldChange)
                         ...
                         verbose = TRUE, ...)
{
  ...
  if(merge)
    ...
    if(length(grp2) == 1)
      ...
      res <- get_diff_1(dds, factor = factor, grp1 = grp1, grp2 = grp2,
                        alpha = alpha, fc = fc, active_exprs = active_exprs,
                        exprs.norm = exprs.norm, verbose = verbose, ...)
  }
  else{
